...
stage('Test') {
				steps {
					sh 'xcodebuild -project PipelineDemo/PipelineDemo -scheme "PipelineDemoTests" -configuration "Debug" build CONFIGURATION_BUILD_DIR=$ABSOLUTE_BUILD_PATH -enableCodeCoverage YES clean test | /usr/local/bin/ocunit2junit'
				}
}
	stage('Upload') {
steps{
